...
export const thumbnailObjectKey = (environment, displayId) => (
  `media/${environment}/admin-thumbs-540-v1/${displayId}.webp`
);
...
export const createAdminThumbnail = async (input) => {
  ...
  let output = await encode(540, 84);
...
SELECT m.id AS display_id,m.owner_user_id,m.task_id,m.business_type,m.business_id,
 ...
 WHERE m.visibility='private'
   ...
   AND m.business_type IN ('member-checkin','meal-checkin','admin-makeup')
...
const saveThumbnailSql = (item, key, thumb, now) => {
  const thumbId = item.thumb_id || `admin-thumb-540-${item.display_id}`;
...
export const runBackfill = async (options) => {
  ...
  const workDir = await mkdtemp(path.join(tmpdir(), 'jinshan20-admin-thumb-540-'));
  try {
    for (const item of candidates) {
      ...
      try {
        ...
        if (thumb.info.format !== 'webp'
            || Math.max(Number(thumb.info.width), Number(thumb.info.height)) > 540) {
        ...
        if (verified.format !== 'webp'
            || Math.max(Number(verified.width || 0), Number(verified.height || 0)) > 540
        ...
        if (!saved || saved.objectKey !== key || saved.mimeType !== 'image/webp'
            || Math.max(Number(saved.width || 0), Number(saved.height || 0)) > 540) {
