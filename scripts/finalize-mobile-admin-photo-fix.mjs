...
const version = '20260730-adminphoto3';
const markerV1 = '/* MOBILE_ADMIN_PHOTO_FIX_V1 */';
const markerV3 = '/* MOBILE_ADMIN_PHOTO_FIX_V3 */';
const privateCacheMarker = '/* PRIVATE_MEDIA_EDGE_CACHE_V3 */';
...

{
  ...
  const next = source.replace(/20260730-(?:flow2|adminphoto1|adminphoto2|adminphoto3)/g, version);
...

{
  ...
  let next = source
    .replace(/const MEDIA_THUMB_MAX_SIZE_MB = (?:0\.12|0\.18);/, 'const MEDIA_THUMB_MAX_SIZE_MB = 0.10;')
    .replace(/const MEDIA_THUMB_QUALITY = (?:0\.72|0\.78|0\.80|0\.82);/, 'const MEDIA_THUMB_QUALITY = 0.78;')
...
function openAdminUserDrawer(studentUser, date) {
  ...
  } else if (!next.includes(markerV3)) {
...

{
  ...
  if (!next.includes(privateCacheMarker)) {
    const privateMediaV3 = `${privateCacheMarker}
...

{
  ...
  const next = source
    .replaceAll('admin-thumbs-540-v1', 'admin-thumbs-540-v3')
    .replace(/let output = await encode\(540, 84\);\n  if \(output\.data\.byteLength > 220 \* 1024\) output = await encode\(540, 76\);\n  if \(output\.data\.byteLength > 220 \* 1024\) output = await encode\(480, 72\);\n  if \(output\.data\.byteLength > 260 \* 1024\) throw new Error\('缩略图压缩后仍超过260KB'\);/,
      "let output = await encode(540, 78);\n  if (output.data.byteLength > 120 * 1024) output = await encode(540, 70);\n  if (output.data.byteLength > 120 * 1024) output = await encode(480, 66);\n  if (output.data.byteLength > 150 * 1024) throw new Error('缩略图压缩后仍超过150KB');")
    .replace("AND (t.id IS NULL OR COALESCE(t.width,0)<500 OR COALESCE(t.mime_type,'')<>'image/webp')",
      "AND (t.id IS NULL OR COALESCE(t.width,0)<500 OR COALESCE(t.mime_type,'')<>'image/webp'\n     OR COALESCE(t.object_key,'') NOT LIKE '%/admin-thumbs-540-v3/%')");
  writeIfChanged(file, source, next);
}

for (const relative of ['public/bootstrap.js', 'public/index.html', 'public/entrance.html']) {
  const { file, source } = readRequired(relative);
  const next = source.replace(/20260730-(?:flow2|adminphoto1|adminphoto2|adminphoto3|plaza640)/g, version);
  writeIfChanged(file, source, next);
}

{
  ...
  fs.writeFileSync(deploymentPath, `${JSON.stringify({
    ...
    feature: 'mobile-admin-thumb-prefetch-v3'
  }, null, 2)}\n`, 'utf8');
}
...
console.log(`Finalized mobile admin photo V3 assets: ${version}.`);
