...
const adminCacheFresh = (savedAt, ttl = 20_000) => Boolean(savedAt && Date.now() - savedAt < ttl);

const adminAccordionMarkup = (name, title, description) => `
  <section class="card admin-compact-module" data-admin-module="${name}">
    <h2 class="admin-accordion-heading">
      <button type="button" class="admin-accordion-trigger" id="adminTrigger-${name}"
        ...
        <span class="admin-module-meta"><i aria-hidden="true">⌄</i></span>
      </button>
    </h2>
...
const renderAdminUserPanel = (completion, result, date) => {
  ...
  document.querySelector('#adminCompactDate').onchange = (event) => {
    ...
    void refreshCompactAdminUsers(event.target.value);
  };
  document.querySelector('#adminUserSearch').onsubmit = (event) => {
    event.preventDefault();
    adminUserQuery = String(new FormData(event.target).get('query') || '').trim();
    adminUserPage = 1;
    sessionStorage.adminUserQuery = adminUserQuery;
    sessionStorage.adminUserPage = '1';
    void refreshCompactAdminUsers(date);
  };
...
async function refreshCompactTeamPanel(force = true) {
  ...
  try {
    ...
    document.querySelectorAll('.edit-team').forEach((button) => {
      button.onclick = () => editTeam(result.teams.find((team) => team.id === button.dataset.id), adminDashboardState.date);
    });
...
const compactPostTile = (post, index) => `
  <button type="button" class="admin-post-tile ${post.status === 'visible' ? 'visible' : 'hidden-post'}" data-id="${post.id}">
    ...
    <span class="user-completion ${post.status === 'visible' ? 'done' : 'pending'}" aria-label="${post.status === 'visible' ? '公开' : '已隐藏'}">${post.status === 'visible' ? '✓' : '—'}</span>
  </button>`;

function openCompactPostActions(post) {
  document.querySelector('#adminPostActionSheet')?.remove();
...
async function refreshCompactPlazaPanel(page = 1) {
  ...
  try {
    ...
    panel.innerHTML = `
      <div class="admin-panel-summary"><span>共 ${result.total} 条帖子</span><small class="muted">点击卡片查看详情和管理</small></div>
      <div class="admin-post-grid">${result.posts.map(compactPostTile).join('') || '<p class="muted">暂无广场帖子</p>'}</div>
      <div class="user-pagination">
    ...
    document.querySelectorAll('.admin-post-tile').forEach((button) => {
...
async function setCompactAdminPanel(name) {
  ...
  modules.forEach((module) => {
    ...
    module.querySelector('.admin-accordion-trigger')?.setAttribute('aria-expanded', String(open));
  ...
  app.innerHTML = `
    ...
    <main class="admin-refactor-shell">
      ...
      <div class="admin-management-stack" aria-label="后台管理模块">
        ...
        ${adminAccordionMarkup('plaza', '活动广场管理', '卡片列表，点击后执行管理操作')}
      </div>
    </main>
