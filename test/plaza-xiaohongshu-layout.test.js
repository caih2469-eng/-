...
test('活动广场卡片只显示文案并采用双列高度平衡布局', () => {
  ...
  assert.match(style, /\.plaza-grid\s*\{[\s\S]*column-gap:\s*4px;/);
  assert.match(style, /\.plaza-column\s*\{[\s\S]*flex-direction:\s*column;[\s\S]*gap:\s*10px;/);
  assert.match(style, /\.plaza-channel-tabs button\s*\{[\s\S]*font-size:\s*13px;/);
  assert.match(style, /\.plaza-card-copy\s*\{[\s\S]*font-size:\s*13px;[\s\S]*font-weight:\s*500;/);
  assert.match(style, /\.plaza-card-meta\s*\{[\s\S]*font-size:\s*11px;/);
});
